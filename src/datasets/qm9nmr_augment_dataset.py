...
def get_atom_type(atom, types):
    ...
    # If it's a carbon atom, determine if it's CH, CH2, or CH3
    elif symbol == 'N':
        num_hydrogens = sum([1 for neighbor in atom.GetNeighbors() if neighbor.GetSymbol() == 'H'])
        if num_hydrogens == 1:
            return types['NH']
    ...
    if bond_type in accepted_bonds_HnC_CHn:
        if atom1.GetSymbol() == 'C' and atom2.GetSymbol() == 'C':
            ...
            if num_hydrogen1 > 0 and num_hydrogen2 > 0:
                ...
                return 5
    # Check O=C bonds
    #if bond_type in accepted_bonds_CO:
        #if (atom1.GetSymbol() == 'O' and atom2.GetSymbol() == 'C') or (
                #atom1.GetSymbol() == 'C' and atom2.GetSymbol() == 'O'):
                #return 2

    return 0

class QM9NMRAugDataset(InMemoryDataset):
    ...
    def raw_file_names(self):
        return "QM9NMR_all_mol_label_v4.pickle"


    @property
    def split_file_name(self):
        return [f'qm9nmr_augment_split_seed{self.split_seed}_train.pickle',
                f'qm9nmr_augment_split_seed{self.split_seed}_val.pickle',
                f'qm9nmr_augment_split_seed{self.split_seed}_test.pickle']
    ...
    def processed_file_names(self):
        return [f'processed_qm9nmr_augment_split_seed{self.split_seed}_train.pt',
                f'processed_qm9nmr_augment_split_seed{self.split_seed}_val.pt',
                f'processed_qm9nmr_augment_split_seed{self.split_seed}_test.pt']
...
class QM9NMRAugDataModule(AbstractDataModule):
    def __init__(self, cfg, split_seed):
        ...
        datasets = {'train': QM9NMRAugDataset(stage='train', root=root_path,
                                           transform=transform, split_seed=split_seed),
                    'val': QM9NMRAugDataset(stage='val', root=root_path,
                                         transform=transform, split_seed=split_seed),
                    'test': QM9NMRAugDataset(stage='test', root=root_path,
                                          transform=transform, split_seed=split_seed)}
        super().__init__(cfg, datasets)


class QM9NMRAuginfos(AbstractDatasetInfos):
